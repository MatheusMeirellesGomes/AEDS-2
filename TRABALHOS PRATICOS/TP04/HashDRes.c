...
/* tabela[0..30] = area principal (tamanho 31)   */
/* tabela[31..49] = area de reserva (tamanho 19) */
...
static int prox_reserva = TAM_TAB; /* proximo slot livre na reserva */
...
void hash_inserir(Restaurante* r) {
    ...
    if (tabela[pos] == NULL) {
        ...
        if (prox_reserva < TAM_TOTAL) {
            tabela[prox_reserva++] = r;
        } else {
            /* reserva cheia: imprime nome do restaurante */
            printf("%s\n", r->nome);
        }
    }
}
...
void hash_pesquisar(char* nome) {
    ...
    /* verifica posicao principal */
    if (tabela[pos] != NULL) {
        comparacoesPesquisa++;
        if (strcmp(tabela[pos]->nome, nome) == 0) {
            char buffer[2048];
            formatar_restaurante(tabela[pos], buffer);
            printf("%d\t%s\n", pos, buffer);
            return;
        }
    }
    /* varre area de reserva */
    for (int i = TAM_TAB; i < prox_reserva; i++) {
        comparacoesPesquisa++;
        if (strcmp(tabela[i]->nome, nome) == 0) {
            char buffer[2048];
            formatar_restaurante(tabela[i], buffer);
            printf("%d\t%s\n", i, buffer);
            return;
        }
    }
...
int main() {
    ...
    for (int i = 0; i < TAM_TOTAL; i++) tabela[i] = NULL;
    ...
    FILE* log = fopen("889989_hash_reserva.txt", "w");
