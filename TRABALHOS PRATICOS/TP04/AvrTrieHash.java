...
public class AvrTrieHash {
    ...
    /* -------- Arvore Trie com Hash (endereçamento aberto, sondagem linear) -------- */
    /* Cada no mantém uma tabela hash de tamanho TAM_HASH mapeando char → NoTrie filho */
    ...
    static class NoTrie {
        ...
        static final int TAM_HASH = 53; /* primo > 26 para baixa taxa de colisao */
        char[] chavesHash;              /* '\0' = slot vazio */
        NoTrie[] filhosHash;

        NoTrie(char c) {
            this.c = c;
            chavesHash = new char[TAM_HASH];   /* inicializado com '\0' (vazio) */
            filhosHash = new NoTrie[TAM_HASH]; /* inicializado com null */
        }

        /* Funcao hash: distribui chars no intervalo [0, TAM_HASH) */
        private int hashChar(char ch) {
            return ((int) ch % TAM_HASH + TAM_HASH) % TAM_HASH;
        }

        NoTrie buscarFilho(char ch) {
            int pos = hashChar(ch);
            int inicio = pos;
            do {
                if (chavesHash[pos] == '\0' && filhosHash[pos] == null) return null;
                if (chavesHash[pos] == ch) return filhosHash[pos];
                pos = (pos + 1) % TAM_HASH;
            } while (pos != inicio);
            return null;
        }

        NoTrie inserirFilho(char ch) {
            int pos = hashChar(ch);
            while (chavesHash[pos] != '\0' || filhosHash[pos] != null) {
                if (chavesHash[pos] == ch) return filhosHash[pos]; /* ja existe */
                pos = (pos + 1) % TAM_HASH;
            }
            NoTrie filho = new NoTrie(ch);
            chavesHash[pos] = ch;
            filhosHash[pos] = filho;
            return filho;
        }
    }
    ...
    static void trieInserir(NoTrie raiz, Restaurante r) {
        ...
        for (int i = 0; i < r.getNome().length(); i++) {
            ...
            if (filho == null) filho = atual.inserirFilho(c);
            atual = filho;
        }
        atual.fim = true;
        atual.restaurante = r;
    ...
    /* Traversal em ordem alfabetica: itera chars de 0 a 127 */
    static void trieEmOrdem(NoTrie no) {
        if (no == null) return;
        if (no.fim) System.out.println(no.restaurante.formatar());
        for (int c = 0; c < 128; c++) {
            NoTrie filho = no.buscarFilho((char) c);
            if (filho != null) trieEmOrdem(filho);
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        PrintWriter pw = new PrintWriter(new FileWriter(MATRICULA + "_arvore_trie_hash.txt"));
