...
/* ---------- Arvore Bicolor / Red-Black (chave: nome) ---------- */
...
void inserir_fixup(No** raiz, No* z) {
    while (z->pai != NULL && z->pai->cor == VERMELHO) {
        if (z->pai->pai != NULL && z->pai == z->pai->pai->esq) {
            ...
            if (tio != NULL && tio->cor == VERMELHO) {
                ...
                if (z == z->pai->dir) {
                    /* Caso 2: tio preto, z filho direito — rotacao esq */
                    z = z->pai;
                    rotacao_esq(raiz, z);
                }
                /* Caso 3: tio preto, z filho esquerdo — rotacao dir */
                z->pai->cor = PRETO;
                z->pai->pai->cor = VERMELHO;
                rotacao_dir(raiz, z->pai->pai);
            }
        } else if (z->pai->pai != NULL) {
            /* Simetrico: pai eh filho direito do avo */
...
No* inserir(No* raiz, Restaurante* r) {
    ...
    /* Insercao BST padrao para encontrar posicao */
    No* atual = raiz;
    No* pai_no = NULL;
    while (atual != NULL) {
        pai_no = atual;
        int cmp = strcmp(r->nome, atual->restaurante->nome);
        if (cmp < 0) atual = atual->esq;
        else if (cmp > 0) atual = atual->dir;
        else { free(novo); return raiz; } /* duplicado */
    }
    novo->pai = pai_no;
    if (strcmp(r->nome, pai_no->restaurante->nome) < 0) pai_no->esq = novo;
    else pai_no->dir = novo;
...
int main() {
    ...
    FILE* log = fopen("889989_arvore_bicolor.txt", "w");
