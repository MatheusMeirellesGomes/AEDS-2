...
/* ---------- Heapsort Parcial (chave: data_abertura; empate: nome; k=10) ---------- */

static int comparar_data(Data* a, Data* b) {
...
static void heapify(Restaurante** arr, int n, int i) {
    ...
    if (maior != i) { trocar(arr, i, maior); heapify(arr, n, maior); }
}

void heapsort_parcial(Restaurante** arr, int n) {
    int lim = n < K ? n : K;

    /* Phase 1: build max-heap of first lim elements */
    for (int i = lim / 2 - 1; i >= 0; i--) heapify(arr, lim, i);
    ...
    /* Phase 3: heapsort the lim-element max-heap -> ascending order */
...
int main() {
    ...
    FILE* log = fopen("889989_heapsort_parcial.txt", "w");
