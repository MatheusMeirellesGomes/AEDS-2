...
public class heapsort {
    ...
    static class Restaurante {
        ...
        public static Restaurante parseRestaurante(String s) {
            ...
            for (int i = 0; i < campos[5].length(); i++)
                if (campos[5].charAt(i) == ';') cont++;

            String[] tipos = new String[cont];
            int t = 0;
            ...
            for (int i = 0; i < campos[6].length(); i++)
                if (campos[6].charAt(i) == '$') faixa++;

            int dash = campos[7].indexOf('-');

            String hA = campos[7].substring(0, dash);
            String hF = campos[7].substring(dash + 1);

            Data data = Data.parseData(campos[8]);
            boolean aberto = campos[9].charAt(0) == 't';

            return new Restaurante(id, nome, cidade, capacidade, avaliacao,
                    tipos, faixa, Hora.parseHora(hA),
                    Hora.parseHora(hF), data, aberto);
        }

        //Procedimento para formatar restaurante.
        public String formatar() {
            StringBuilder sb = new StringBuilder();

            sb.append("[").append(id).append(" ## ").append(nome)
              .append(" ## ").append(cidade).append(" ## ").append(capacidade)
              .append(" ## ").append(String.format("%.1f", avaliacao))
              .append(" ## [");
            ...
            sb.append(" ## ").append(horarioAbertura.formatar())
              .append("-").append(horarioFechamento.formatar())
              .append(" ## ").append(dataAbertura.formatar())
              .append(" ## ").append(aberto).append("]");
    ...
    static int comparar(Restaurante a, Restaurante b) {
        ...
        if (a.getDataAbertura().getAno() != b.getDataAbertura().getAno())
            return a.getDataAbertura().getAno() - b.getDataAbertura().getAno();

        if (a.getDataAbertura().getMes() != b.getDataAbertura().getMes())
            return a.getDataAbertura().getMes() - b.getDataAbertura().getMes();

        if (a.getDataAbertura().getDia() != b.getDataAbertura().getDia())
            return a.getDataAbertura().getDia() - b.getDataAbertura().getDia();
    ...
    static void troca(Restaurante[] arr, int i, int j) {
