...
public class ArvoreAvl {
    ...
    /* ---------- Arvore AVL (chave: nome) ---------- */

    static class No {
        Restaurante restaurante;
        No esq, dir;
        int altura;
        No(Restaurante r) { restaurante = r; altura = 1; }
    }
    ...
    static void atualizarAltura(No no) {
        if (no != null) {
            int he = altura(no.esq), hd = altura(no.dir);
    ...
    static No balancear(No no) {
        ...
        /* LL: rotacao simples a direita */
        if (fb > 1 && fatorBalanceamento(no.esq) >= 0)
            return rotacaoDir(no);
        /* LR: rotacao dupla esq-dir */
        if (fb > 1 && fatorBalanceamento(no.esq) < 0) {
            no.esq = rotacaoEsq(no.esq);
            return rotacaoDir(no);
        }
        /* RR: rotacao simples a esquerda */
        if (fb < -1 && fatorBalanceamento(no.dir) <= 0)
            return rotacaoEsq(no);
        /* RL: rotacao dupla dir-esq */
        if (fb < -1 && fatorBalanceamento(no.dir) > 0) {
    ...
    static No inserir(No raiz, Restaurante r) {
        if (raiz == null) return new No(r);
    ...
    static String pesquisar(No raiz, String nome) {
        if (raiz == null) return "NAO";
        StringBuilder sb = new StringBuilder("raiz");
        No atual = raiz;
        while (atual != null) {
            comparacoesPesquisa++;
            int cmp = nome.compareTo(atual.restaurante.getNome());
            if (cmp == 0) { sb.append(" SIM"); return sb.toString(); }
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (sc.hasNextInt()) {
            ...
            for (int i = 0; i < tamanho; i++) {
                if (todos[i].getId() == id) { raiz = inserir(raiz, todos[i]); break; }
            }
        }
        ...
        PrintWriter pw = new PrintWriter(new FileWriter(MATRICULA + "_arvore_avl.txt"));
